...
int main(int argc, char* argv[]) {
    ...
    while(arq.get(ch)){
        if(iswalpha(ch)){               //Verifica se o caractere é uma letra
            palavra += towlower(ch);    //Concatena a letra minúscula
            num_caracteres++;
        } else if(palavra.size() > 0){  //Verifica se a palavra tem conteúdo
            palavras[palavra]++;        //Incrementa a contagem da palavra
            num_palavras++;
            palavra.clear();            //Limpa a palavra para a próxima
        }
    }
